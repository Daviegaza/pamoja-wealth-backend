...
export async function getById(chamaId: string) {
  ...
  return {
    chama: {
      ...
      targetAmount: chama.targetAmount ? Number(chama.targetAmount) : null,
      raisedAmount: chama.raisedAmount ? Number(chama.raisedAmount) : 0,
      deadline: chama.deadline?.toISOString() || null,
      status: chama.status,
      memberCount: chama.memberships.length,
